...
interface TimelineClipBlockProps {
  ...
  onReorder: (provisionalTimelineStart: number) => void
}
...
export function TimelineClipBlock({
  ...
  const onBodyPointerUp = (event: React.PointerEvent) => {
    ...
    onReorder(Math.max(0, clip.timelineStart + deltaTime))
    setDragging(false)
    setDragDeltaPx(0)
  ...
  return (
    <div
      className={`timeline-clip ${selected ? 'timeline-clip-selected' : ''} ${dragging ? 'timeline-clip-dragging' : ''}`}
      style={{ left, width }}
      onPointerDown={onBodyPointerDown}
      onPointerMove={onBodyPointerMove}
      onPointerUp={onBodyPointerUp}
    >
      <span className="timeline-clip-label">Clip</span>
      <div
